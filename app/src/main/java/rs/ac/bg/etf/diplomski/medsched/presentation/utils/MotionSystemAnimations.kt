...
val SharedXAxisEnterTransition: (Density) -> EnterTransition = { density ->
    fadeIn(animationSpec = tween(durationMillis = 300, delayMillis = 180, easing = LinearOutSlowInEasing)) +
...
val SharedXAxisExitTransition: (Density) -> ExitTransition = { density ->
    fadeOut(animationSpec = tween(durationMillis = 180, easing = FastOutLinearInEasing)) +
            slideOutHorizontally(animationSpec = tween(durationMillis = 400)) {
                with(density) { (-80).dp.roundToPx() }
            }
}
...
val FadeThroughEnterTransition =
    fadeIn(animationSpec = tween(durationMillis = 300, delayMillis = 180, easing = LinearOutSlowInEasing)) +
            scaleIn(initialScale = 0.92f, animationSpec = tween(durationMillis = 300, delayMillis = 180, easing = LinearOutSlowInEasing))

val FadeThroughExitTransition = fadeOut(animationSpec = tween(durationMillis = 180, easing = FastOutLinearInEasing))
